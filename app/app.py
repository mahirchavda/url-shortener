...
def handle_all():
    ...
    try:
        ...
        logger.info(ACCESS_LOG.format(request.remote_addr, request.method,
                                      request.path, "Redirecting to::{}".format(url), request.user_agent))
        return redirect(url)
    except ShortURLNotFound as err:
        logger.info(ACCESS_LOG.format(request.remote_addr, request.method,
                                      request.path, "Not Found::{}".format(err), request.user_agent))
        return str(err)
    except Exception as err:
        response = "Unknown error occured: {}".format(err)
        logger.error(ACCESS_LOG.format(request.remote_addr, request.method,
                                       request.path, "Error::{}".format(response), request.user_agent))
